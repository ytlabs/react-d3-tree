...
This is a fork from original project [react-3d-tree](https://github.com/bkrem/react-d3-tree). It has several additional features we require in our project.

- __Handles node selection:__ It keeps track of the most recently clicked node and applies styling to that node.
- __Override default click behaviour:__ [Original project](https://github.com/bkrem/react-d3-tree) toggles collapse state of the node internally, after that it calls `onClick` callback. We have modified it to optionally execute the default click behaviour. To preserve original behaviour we have introduced additional callback prop `onClickCompleted`.
- __Tooltip foreign object:__ [Original project](https://github.com/bkrem/react-d3-tree) has two mutually exclusive options, you may either have default label, or use foreign object to render label. One can implement label + tooltip using foreign object. However we want to keep default label rendering and render tooltip as standalone foreign object. We have linked it to node selection so that tooltip foreign object is rendered only for selected node. Tooltip foreign object has two additional props, `onClick` and `onDeselect`. `onClick` prop is used internally by `Node` and it only prevents click event propagation. `onDeselect` is available for the component given to `render` field of `nodeTooltipComponent`. When `onDeselect` is called inside tooltip component, it deselects the node in the tree. `Tree` component has also `onDeselect` prop, which is invoked when tooltip component fires deselect action.   Rendering tooltip can be bound to right click action by `showTooltipOnCtxMenu` flag.
...
| `nodeTooltipComponent`        | `object`               | see [Using foreignObjects](#using-foreignobjects)                             |           | `null`                                                        | Allows using a additional React component as a node tooltip; attached to the selected node. Requires `allowForeignObjects` and `shouldHandleNodeSelection` to be set.                                                                                                                                                                                                                                                                                        |
| `onClick`                     | `func`                 |                                                                               |           | `undefined`                                                   | Callback function to be called when a node is clicked. <br /><br />Has the function signature `(nodeData, evt, previousNodeData)`. The clicked node's data object is passed as first parameter, event object as second. It is called before default handling procedures. By default click event toggles node's collapse state. If this callback returns explicitly `false` then it prevents default behaviour.   |
| `onClickCompleted`            | `func`                 |                                                                               |           | `undefined`                                                   | Callback function to be called when a library finishes handling `onClick` event. <br /><br />It functions just like former `onClick` callback, ie. it is called after completion of default handling procedures                  |
...
| `onDeselect`                    | `func`                 |                                                                               |           | `undefined`                                                   | Callback function to be called when the tooltip component request selected node to be deselected.                                                                                                                                                                                  |
...
| `shouldHandleNodeSelection`   | `bool`                 |                                                                               |           | `false`                                                       | Keeps track of most recently clicked node. Applies `selectedNode` style to selected node.     |
| `showTooltipOnCtxMenu`   | `bool`                 |                                                                               |           | `false`                                                       | Renders tooltip on selected node if only node is right clicked     |
